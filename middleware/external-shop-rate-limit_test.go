...
func TestExternalShopRefreshRateLimitUsesUserScope(t *testing.T) {
	...
	router.POST("/refresh", ExternalShopRefreshRateLimit(), func(c *gin.Context) {
		c.Status(http.StatusNoContent)
	})

	for attempt := 1; attempt <= externalShopRefreshRateLimitNum; attempt++ {
	...
	blockedReq := httptest.NewRequest(http.MethodPost, "/refresh", nil)
	...
	secondUserRouter.POST("/refresh", ExternalShopRefreshRateLimit(), func(c *gin.Context) {
		c.Status(http.StatusNoContent)
	})

	secondUser := httptest.NewRecorder()
	secondUserReq := httptest.NewRequest(http.MethodPost, "/refresh", nil)
