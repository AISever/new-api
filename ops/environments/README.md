...
> 重要：**当前真实对外生产环境是新 `kkidc` 主机 `114.66.47.192` 上的 `https://api.aisever.cn`**。
...
1. **功能发布顺序**：先 `kkidc` 测试环境，之后再进新 `kkidc` 生产环境。
2. **部署前先看本目录**：需要判断“该操作到底影响哪个环境”时，先读本目录，再进入对应环境文档。
3. **新 `kkidc` 服务器**：使用 `ops/scripts/kkidc-host-deploy.sh production|test`，默认读取 `.kkidc/.env.lighthouse` 和 `.env.local`，并只部署当前分支已提交的 `HEAD`。
