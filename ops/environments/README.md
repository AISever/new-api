...
> 重要：**当前真实对外生产环境仍然是 `kkidc` 生产环境**。`tencent` 服务器虽然已经完成部署，但因为域名/HTTPS 切换未完成，**尚未启用对外生产流量**。

| 环境 | 当前状态 | 访问地址 | 服务器 | 容器 | 数据库 | 用途 |
| --- | --- | --- | --- | --- | --- | --- |
| `kkidc` 生产环境 | **启用中** | `http://202.140.142.149:3000` | `202.140.142.149` | `new-api-local` | `new-api` | 当前真实生产流量 |
...
1. **功能发布顺序**：先 `kkidc` 测试环境，必要时再走 `tencent` 测试环境，之后再进 `kkidc` 生产环境；`tencent` 预备生产环境当前只做迁移、验证、切换准备。
