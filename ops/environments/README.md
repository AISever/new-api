...
> 重要：**当前真实对外个人生产环境仍是新 `kkidc` 主机 `114.66.47.192` 上的 `https://api.aisever.cn`；企业生产环境与其共享主机但独立容器运行，公网域名切流前可先通过 `http://114.66.47.192:3002` 做独立验证。**
...
| `kkidc` 企业生产环境 | **独立部署 / 待公网切流** | 建议 `https://corp-api.aisever.cn`，切流前直连 `http://114.66.47.192:3002` | `114.66.47.192` | `new-api-enterprise` | `new-api-enterprise` | 企业客户独立生产流量 |
...
4. **共享主机上的企业首发**：先备份个人生产环境；若 `ENTERPRISE_HOSTNAME` 尚未配置，企业环境先按 `:3002` 启动，不应影响 `https://api.aisever.cn`。
