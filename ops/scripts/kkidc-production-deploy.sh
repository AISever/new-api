#!/bin/bash
#
# New-API kkidc 生产部署脚本
# 支持本地部署和远程服务器部署
#
# 使用方法: ./ops/scripts/kkidc-production-deploy.sh [模式] [选项]
#
# 模式:
#   local           本地部署（默认）
...
#
# 示例:
#   ./ops/scripts/kkidc-production-deploy.sh local                    # 本地部署
#   ./ops/scripts/kkidc-production-deploy.sh remote --clear-cache    # 远程部署并清除缓存
#   ./ops/scripts/kkidc-production-deploy.sh local --skip-build      # 本地只重启容器
#
