...
# New-API kkidc 测试环境部署脚本
# 用于在服务器上部署独立的 kkidc 测试环境（端口 3001）
...
start_container() {
    ...
    print_success "kkidc 测试容器启动完成"
}

# 清除 kkidc 测试环境的 Redis 缓存
clear_redis_cache() {
    print_step "清除 kkidc 测试环境 Redis 缓存 (db 1)..."
