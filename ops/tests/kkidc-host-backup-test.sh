...
set +e
ENTERPRISE_OUTPUT="$(bash "$SCRIPT_UNDER_TEST" enterprise --config "$CONFIG_FILE" --dry-run 2>&1)"
ENTERPRISE_EXIT="$?"
set -e
...
if ! printf '%s\n' "$ENTERPRISE_OUTPUT" | grep -q '^pg_db=new-api-enterprise$'; then
  echo "FAIL: enterprise backup should target new-api-enterprise database" >&2
  exit 1
fi

if ! printf '%s\n' "$ENTERPRISE_OUTPUT" | grep -q '^backup_root=/opt/new-api-enterprise-backups$'; then
  echo "FAIL: enterprise backup root mismatch" >&2
  exit 1
fi
