...
git -C "$REPO_DIR" push -u origin codex/release-v0.11.2-patch.2-p1 >/dev/null
...
ENTERPRISE_HOSTNAME_ALIASES=enterprise.aisever.cn
EOF
...
if ! printf '%s\n' "$PROD_OUTPUT" | grep -q '^deploy_git_remote=origin$'; then
  echo "FAIL: production deploy should record git remote" >&2
  exit 1
fi

if ! printf '%s\n' "$PROD_OUTPUT" | grep -q '^deploy_git_ref=origin/codex/release-v0.11.2-patch.2-p1$'; then
  echo "FAIL: production deploy should verify pushed origin branch" >&2
  exit 1
fi

if ! printf '%s\n' "$PROD_OUTPUT" | grep -q '^head_pushed_verified=true$'; then
  echo "FAIL: production deploy should require pushed HEAD" >&2
  exit 1
fi
...
if ! printf '%s\n' "$ENTERPRISE_OUTPUT" | grep -q '^target_env=enterprise$'; then
  echo "FAIL: enterprise output missing target env" >&2
  exit 1
fi

if ! printf '%s\n' "$ENTERPRISE_OUTPUT" | grep -q '^app_container=new-api-enterprise$'; then
  echo "FAIL: enterprise container mismatch" >&2
  exit 1
fi

if ! printf '%s\n' "$ENTERPRISE_OUTPUT" | grep -q '^app_port=3002$'; then
  echo "FAIL: enterprise port mismatch" >&2
  exit 1
fi

if ! printf '%s\n' "$ENTERPRISE_OUTPUT" | grep -q '^pg_db=new-api-enterprise$'; then
  echo "FAIL: enterprise database mismatch" >&2
  exit 1
fi

if ! printf '%s\n' "$ENTERPRISE_OUTPUT" | grep -q '^redis_conn_string=redis://new-api-redis/2$'; then
  echo "FAIL: enterprise redis db mismatch" >&2
  exit 1
fi
...
if ! printf '%s\n' "$ENTERPRISE_OUTPUT" | grep -q '^public_hostnames=corp-api.aisever.cn,enterprise.aisever.cn$'; then
  echo "FAIL: enterprise public hostnames mismatch" >&2
  exit 1
fi

if ! printf '%s\n' "$ENTERPRISE_OUTPUT" | grep -q '^caddy_site_addresses=corp-api.aisever.cn, enterprise.aisever.cn$'; then
  echo "FAIL: enterprise caddy site addresses mismatch" >&2
  exit 1
fi
