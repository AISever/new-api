...
python3 - "$PROFILE_UNDER_TEST" "$PROFILE_COPY" <<'PY'
import pathlib
import sys

src = pathlib.Path(sys.argv[1]).read_text(encoding="utf-8")
src = src.replace("base_url: https://yunwu.ai", "base_url: http://127.0.0.1:19091")
...
if [ "$EXIT_CODE" -ne 0 ]; then
  echo "FAIL: import with probes and channel keys should succeed" >&2
  echo "$OUTPUT" >&2
  exit 1
fi
...
if ! printf '%s\n' "$OUTPUT" | grep -q '"passed": 3'; then
  echo "FAIL: all three family probes should pass" >&2
  echo "$OUTPUT" >&2
  exit 1
fi

if ! printf '%s\n' "$OUTPUT" | grep -q '"provided_channel_key_families"'; then
  echo "FAIL: output should include provided channel key families" >&2
  echo "$OUTPUT" >&2
  exit 1
fi
...
FAIL_OUTPUT="$(
  python3 "$SCRIPT_UNDER_TEST" \
    ...
    --require-channel-keys \
    --channel-key kling=kling-secret \
    --channel-key vidu=sk-vidu-secret 2>&1
)"
FAIL_EXIT_CODE="$?"
set -e

if [ "$FAIL_EXIT_CODE" -eq 0 ]; then
  echo "FAIL: missing doubao-video key should fail when channel keys are required" >&2
  echo "$FAIL_OUTPUT" >&2
  exit 1
fi

if ! printf '%s\n' "$FAIL_OUTPUT" | grep -q 'doubao-video'; then
  echo "FAIL: missing key failure should mention doubao-video" >&2
  echo "$FAIL_OUTPUT" >&2
  exit 1
fi

echo "PASS: channel onboarding importer supports explicit channel keys and upstream family probes"
