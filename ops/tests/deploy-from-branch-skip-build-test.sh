...
if grep -q "compose .* build " "$DOCKER_LOG"; then
  echo "FAIL: BUILD_IMAGE=false should not run docker compose build" >&2
  exit 1
fi

grep -q "compose -f docker-compose.yml -f docker-compose.override.yml -p dryrun up -d --no-build" "$DOCKER_LOG"

echo "PASS: deploy-from-branch skips build and runs up --no-build"
