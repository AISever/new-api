...
SCRIPT_UNDER_TEST="${1:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/scripts/kkidc-test-yunwu-onboarding.sh}"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/setup":
            ...
            return
        if path == "/api/status":
            self._send_json({"success": True, "data": {"status": True, "setup": True}})
            return
        if path == "/api/option/":
...
python3 "$TARGET_SERVER_SCRIPT" "$TARGET_PORT_FILE" >/dev/null 2>&1 &
TARGET_PID="$!"
python3 "$UPSTREAM_SERVER_SCRIPT" "$UPSTREAM_PORT_FILE" >/dev/null 2>&1 &
UPSTREAM_PID="$!"
...
python3 - "$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/channel-onboarding/profiles/yunwu.yaml" "$PROFILE_COPY" "$UPSTREAM_PORT" "$TARGET_PORT" <<'PY'
import pathlib
import sys

src = pathlib.Path(sys.argv[1]).read_text(encoding="utf-8")
src = src.replace("base_url: https://yunwu.ai", f"base_url: http://127.0.0.1:{sys.argv[3]}")
src = src.replace("test_status_url: http://114.66.47.192:3001/api/status", f"test_status_url: http://127.0.0.1:{sys.argv[4]}/api/status")
...
if [ "$EXIT_CODE" -ne 0 ]; then
  echo "FAIL: yunwu onboarding wrapper should succeed" >&2
  echo "$OUTPUT" >&2
  exit 1
fi

if ! printf '%s\n' "$OUTPUT" | grep -q '"probe_summary"'; then
  echo "FAIL: output should include probe summary" >&2
  echo "$OUTPUT" >&2
  exit 1
fi

if ! printf '%s\n' "$OUTPUT" | grep -q '"test_status_check"'; then
  echo "FAIL: output should include test status check result" >&2
  echo "$OUTPUT" >&2
  exit 1
fi

if ! printf '%s\n' "$OUTPUT" | grep -q '"target_channels_created": 3'; then
  echo "FAIL: wrapper should create three channels" >&2
  echo "$OUTPUT" >&2
  exit 1
fi

echo "PASS: kkidc test yunwu onboarding wrapper imports, probes, and verifies test status"
