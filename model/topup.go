...
func RechargeLdxp(tradeNo string, providerPayload string) (err error) {
	if tradeNo == "" {
		return errors.New("未提供支付单号")
	}
	...
	if err != nil {
		common.SysError("ldxp topup failed: " + err.Error())
		return errors.New("充值失败，请稍后重试")
	}

	if quotaToAdd > 0 {
		RecordLog(topUp.UserId, LogTypeTopup, fmt.Sprintf("LDXP充值成功，充值额度: %v，支付金额: %.2f", logger.FormatQuota(quotaToAdd), topUp.Money))
	}
