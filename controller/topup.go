...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if topUp.Status == common.TopUpStatusPending {
			...
			if err = model.GrantInviteRatioRewardForTopUp(topUp.UserId, topUp.Money); err != nil {
				logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 邀请比例返奖失败 trade_no=%s user_id=%d client_ip=%s money=%.2f error=%q", topUp.TradeNo, topUp.UserId, c.ClientIP(), topUp.Money, err.Error()))
			}
		}
	} else {
