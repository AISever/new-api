...
paths:
  /mj/submit/shorten:
    post:
      summary: 6.缩短提示词（Shorten）接口
      deprecated: false
      description: 执行shorten操作，识别在关键词中有哪些单词在生成图像时发挥关键因素，哪些单词则不太重要。
      ...
      requestBody:
        content:
          application/json:
            schema:
              $ref: >-
                #/components/schemas/Describe%E6%8F%90%E4%BA%A4%E5%8F%82%E6%95%B0
            example:
              mode: RELAX
              prompt: Cat
      responses:
      ...
      x-run-in-apifox: https://app.apifox.com/web/project/3593080/apis/api-307620342-run
components:
  schemas:
    Describe提交参数:
      type: object
